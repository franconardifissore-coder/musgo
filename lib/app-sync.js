(function attachMusgoAppSync(globalScope) {
  ...
  // Uploads a plant image File to Supabase Storage.
  // Returns the public URL, or null if cloud sync is not enabled.
  async function uploadPlantImage(plantId, userId, file) {
    if (!isCloudSyncEnabled()) return null;
    return globalScope.supabaseData.uploadPlantImage(plantId, userId, file);
  }

  // Deletes a plant image from Supabase Storage given its public URL.
  // No-op if cloud sync is not enabled or if the URL is not a Storage URL.
  async function deletePlantImage(imageUrl) {
    if (!isCloudSyncEnabled()) return;
    return globalScope.supabaseData.deletePlantImage(imageUrl);
  }
