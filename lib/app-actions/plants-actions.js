(function attachMusgoPlantActions(globalScope) {
  ...
  function openPlantDetail(plantId) {
    ...
    // Always land users on the Riegos tab when opening a plant.
    state.detailActiveTab = 'riegos';
    ...
    // El schema actual de Supabase no persiste `metadata` ni `identifiedCommonName`,
    // así que después de un reload los plants vienen "pelados". Si el plant fue
    // identificado y no tenemos metadata en memoria, la pedimos lazy. La edge
    // function cachea por nombre científico, así que es prácticamente instantáneo.
    ensurePlantMetadataLoaded(plant);
  }

  // Hidrata `plant.metadata` (luz / riego / origen / common_name) usando
  // plantMetadataClient. No-op si la planta ya tiene metadata, no fue identificada,
  // o el cliente no está disponible.
  function ensurePlantMetadataLoaded(plant) {
    const { render } = getPlantActionsContext();
    const client = globalScope.plantMetadataClient;
    if (!client || typeof client.fetchPlantMetadata !== 'function') return;
    if (!plant) return;
    // Re-fetcheamos si la metadata existe pero no tiene fun_fact: significa
    // que fue persistida en localStorage antes del backfill.
    if (plant.metadata && plant.metadata.fun_fact) return;
    if (plant.metadataLoading) return;

    const scientificName = plant.identifiedSpecies || plant.species;
    if (!scientificName || typeof scientificName !== 'string') return;

    plant.metadataLoading = true;
    plant.metadataError = null;
    render();

    client.fetchPlantMetadata(scientificName)
      .then((response) => {
        const state = getStateSnapshot();
        const current = state.plants.find((p) => p.id === plant.id);
        if (!current) return;
        current.metadata = response && response.metadata ? response.metadata : null;
        current.metadataLoading = false;
        current.metadataError = null;
        render();
      })
      .catch((error) => {
        const state = getStateSnapshot();
        ...
        if (!current) return;
        current.metadataLoading = false;
        current.metadataError = error && error.code ? error.code : 'metadata_failed';
        render();
      });
  }

  // Switches the active tab inside the plant detail screen.
  // Tab values: 'riegos' | 'detalles' | 'evolucion'.
  ...
  // Frecuencias disponibles en el dropdown de la UI. Si Haiku recomienda
  // un valor que no está en esta lista, lo snappeamos al más cercano.
  const FREQ_OPTIONS = [1, 2, 3, 5, 7, 14, 30];

  function snapToFreqOption(days) {
    const n = Number(days);
    if (!Number.isFinite(n) || n <= 0) return null;
    let best = FREQ_OPTIONS[0];
    let bestDiff = Math.abs(FREQ_OPTIONS[0] - n);
    for (let i = 1; i < FREQ_OPTIONS.length; i++) {
      const d = Math.abs(FREQ_OPTIONS[i] - n);
      if (d < bestDiff) {
        best = FREQ_OPTIONS[i];
        bestDiff = d;
      }
  ...
  // Lanza la carga de metadata para una planta identificada y va
  // actualizando el draft activo a medida que llega la respuesta.
  // No bloquea la navegación: la pantalla se abre con loading=true y
  // los datos se hidratan cuando responden.
  function fetchAndApplyPlantMetadata(scientificName, draftRef) {
    const { render } = getPlantActionsContext();
    const client = globalScope.plantMetadataClient;
    if (!client || typeof client.fetchPlantMetadata !== 'function') return;
    if (!scientificName || typeof scientificName !== 'string') return;

    draftRef.metadataLoading = true;
    draftRef.metadataError = null;

    client.fetchPlantMetadata(scientificName)
      .then((response) => {
        const state = getStateSnapshot();
        // Solo aplicamos si el draft sigue siendo el mismo (evita pisar
        // datos si el usuario abrió otra identificación en el medio).
        if (state.screenPlantDraft !== draftRef) return;

        draftRef.metadata = response && response.metadata ? response.metadata : null;
        draftRef.metadataLoading = false;
        draftRef.metadataError = null;

        // Auto-fill de la frecuencia con la recomendación, solo si el
        // usuario todavía no eligió manualmente otra.
        const recommended = draftRef.metadata && draftRef.metadata.watering_freq_days;
        if (recommended && !draftRef.freqUserChanged) {
          const snapped = snapToFreqOption(recommended);
          if (snapped) draftRef.freq = snapped;
        }
      ...
      .catch((error) => {
        ...
        draftRef.metadataLoading = false;
        draftRef.metadataError = error && error.code ? error.code : 'metadata_failed';
        render();
      });
  }
  ...
  function openPlantCreateView(prefill = null) {
    ...
    const state = getStateSnapshot();
    const draft = createPlantDraft(prefill);
    state.screenPlantDraft = draft;

    // Si venimos de una identificación, arrancamos el fetch de metadata
    // en paralelo (la nav a la pantalla no se bloquea).
    if (prefill && prefill.identifiedSpecies) {
      fetchAndApplyPlantMetadata(prefill.identifiedSpecies, draft);
    }
  ...
  function updateScreenPlantField(field, value) {
    ...
    if (field === 'freq') {
      state.screenPlantDraft.freqUserChanged = true;
    }
  }
  ...
  async function waterPlant(id) {
    ...
    showToast(`Regando "${plant.name}"…`, { loading: true });
  ...
  // ===== MAIN PLANT PHOTO (profile image) =====

  async function updatePlantMainPhoto(plantId, file) {
    const { uploadPlantImage, syncPlantToCloud, render, showToast } = getPlantActionsContext();
    const state = getStateSnapshot();
    ...
    try {
      ...
      const url = await uploadPlantImage(plantId, userId, jpegFile);
      plant.imagePreview = url;
      render();
      await syncPlantToCloud(plant);
      showToast('🌿 Foto principal actualizada');
    } catch (err) {
      console.error('[photos] Main photo update failed', err);
      showToast('⚠️ No pudimos actualizar la foto principal');
    }
  }
  ...
  globalScope.updatePlantMainPhoto = updatePlantMainPhoto;
  ...
  globalScope.setDetailTab = setDetailTab;
