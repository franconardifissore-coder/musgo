(function attachMusgoSpacesActions(globalScope) {
  ...
  async function saveSectionChanges() {
    ...
    section.outdoor = ['balcón', 'terraza', 'jardín', 'exterior', 'patio'].some((word) => nextName.toLowerCase().includes(word));
    state.screenSectionDraft = null;
    state.editingSectionId = null;
  ...
  async function saveSectionFromScreen() {
    ...
    const section = {
      ...
      name: nextName,
      icon: draft.icon || '🪴',
      outdoor: ['balcón', 'terraza', 'jardín', 'exterior', 'patio'].some((word) => nextName.toLowerCase().includes(word)),
    };
