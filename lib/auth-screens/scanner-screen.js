(function attachMusgoScannerScreen(globalScope) {
  ...
  function renderMatchCards(results, escapeHtml) {
    ...
    const cards = results.map((result, i) => {
      ...
      const visual = refImg
        ? `
            <div class="scan-match-photo-wrap">
              <img class="scan-match-photo" src="${escapeHtml(refImg.url)}" alt="${escapeHtml(result.scientificName)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
              <div class="scan-match-photo scan-match-photo--empty" style="display:none">🌿</div>
            </div>
          `
      ...
      const commonNameValue = result.commonNames && result.commonNames[0]
        ...
        : '';
      const confidencePercent = Math.round(Number(result.confidence) || 0);

      return `
        <button class="scan-match-card" type="button" onclick="selectIdentificationResult(${i})">
          <div class="scan-match-visual">
            ${visual}
          </div>
          <div class="scan-match-chip">
            <img class="scan-match-chip-icon" src="/resources/check-icon.svg" alt="" aria-hidden="true">
            <span class="scan-match-chip-text">Match con ${confidencePercent}% de confianza</span>
          </div>
          <div class="scan-match-content">
            <div class="scan-match-species">${speciesValue}</div>
            ${commonNameValue ? `<div class="scan-match-common">${commonNameValue}</div>` : ''}
          </div>
        </button>
      `;
    }).join('');
  ...
  function renderScanScreen() {
    ...
    // En el estado "con resultados" el copy descriptivo se omite para
    // dejar que el título quede en una sola línea junto al thumbnail.
    // En loading/error/inicial seguimos mostrando el texto guía.
    const scanCopy = state.identification.loading
      ? 'Estamos revisando la imagen para identificar tu planta automáticamente.'
      : (state.identification.error || (hasResults ? '' : 'Reconocimiento con AI'));
        ...
        <div class="scan-panel">
          ...
          <button class="scan-box ${state.identification.loading ? 'is-loading' : ''} ${hasResults ? 'has-results' : ''}" type="button" onclick="openDirectScanPicker()">
            <div class="scan-box-inner">
              ...
              ${scanCopy ? `<div class="scan-box-copy ${state.identification.loading ? 'is-loading' : ''} ${state.identification.error ? 'is-error' : ''}">${scanCopy}</div>` : ''}
            </div>
          </button>
          ...
          ${!hasResults ? `<button class="scan-secondary" type="button" onclick="openPlantCreateView()">Crear manualmente</button>` : ''}
        </div>
      </section>
    `;
  }
