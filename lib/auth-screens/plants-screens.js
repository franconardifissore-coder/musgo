(function attachMusgoPlantsScreens(globalScope) {
  ...
  // Renderiza la tarjeta "¿Sabías que?" con el fun_fact generado por Haiku.
  // Comparte el mismo ciclo de loading/error que renderIdentifiedPlantMetadata:
  // si metadataLoading está en true muestra skeletons, si no hay fun_fact
  // (plantas sin identificar, o rows cacheados antes de esta feature) no
  // muestra nada.
  function renderPlantFunFact(draft, escapeHtml) {
    ...
    const funFact = draft.metadata && draft.metadata.fun_fact;

    // No mostramos la sección si el fetch terminó pero no hay dato
    // (planta manual, o cache viejo sin fun_fact).
    if (!isLoading && !funFact) return '';

    return `
      <div class="plant-fun-fact">
        <span class="plant-fun-fact-label">¿Sabías que?</span>
        ${isLoading
          ? `<span class="plant-fun-fact-skeleton" aria-hidden="true"></span>
             <span class="plant-fun-fact-skeleton" aria-hidden="true"></span>`
          : `<p class="plant-fun-fact-text">${escapeHtml(funFact)}</p>`
        }
      </div>
    `;
  }
      ...
      <section class="screen editor-screen plant-create-screen">
        ...
        ${isIdentified ? `
          <div class="identified-plant-card">
            ...
            ${renderPlantFunFact(draft, escapeHtml)}
          </div>
          <div class="identified-plant-form">
            <div class="form-group">
              <label>Ponle nombre a tu planta</label>
            ...
            <div class="form-group">
              <label>Frecuencia de riego sugerida</label>
    ...
    const detallesTabHtml = `
      <div class="plant-detail-tab-panel" role="tabpanel">
        ...
        <div class="identified-plant-card identified-plant-card--editable">
          ...
          ${renderPlantFunFact(plant, escapeHtml)}
        </div>
      </div>
    `;
