(function attachMusgoPlantsScreens(globalScope) {
  ...
  function renderIdentifiedPlantMetadata(draft, escapeHtml) {
    ...
    const rows = [
      { icon: '☀️', label: 'Luz', value: m.light ? formatLightValue(m.light) : null },
      { icon: '💧', label: 'Riego', value: m.watering_level || m.watering_freq_days ? formatWateringValue(m.watering_level, m.watering_freq_days) : null },
      { icon: '🌍', label: 'Origen', value: m.origin || null },
    ];
      ...
      <div class="identified-plant-metadata${isLoading ? ' identified-plant-metadata--loading' : ''}">
        ${rows.map((row) => `
          <div class="identified-plant-metadata-row">
            <span class="identified-plant-metadata-icon" aria-hidden="true">${row.icon}</span>
