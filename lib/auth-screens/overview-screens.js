(function attachMusgoOverviewScreens(globalScope) {
  ...
  function renderPushButton() {
    const push = globalScope.musgoPush;
    if (!push) return `<p class="push-denied-hint">[debug] musgoPush no cargó</p>`;
    if (!push.isSupported()) return `<p class="push-denied-hint">[debug] Push no soportado en este dispositivo/browser</p>`;

    const status = push.getPermissionStatus();
    if (status === 'granted') {
      return `<button class="btn-secondary" type="button" onclick="handleDisablePush()">Desactivar recordatorios de riego</button>`;
    }
    if (status === 'denied') {
      return `<p class="push-denied-hint">Las notificaciones están bloqueadas. Habilitálas desde Configuración → Musgo.</p>`;
    }
    // 'default' — todavía no se pidió permiso
    return `<button class="btn-primary" type="button" onclick="handleEnablePush()">🔔 Activar recordatorios de riego</button>`;
  }
