(function attachPushNotifications(global) {
  ...
  // Cache del estado real de suscripción (null = sin suscripción, object = suscrito, undefined = no chequeado aún)
  let _subscriptionCache = undefined;

  // Verifica la suscripción real en PushManager y re-renderiza si cambió
  async function refreshSubscriptionCache() {
    if (!isSupported()) { _subscriptionCache = null; return; }
    const reg = await getRegistration();
    const sub = reg ? await reg.pushManager.getSubscription() : null;
    const changed = (_subscriptionCache === undefined) || (!!_subscriptionCache !== !!sub);
    _subscriptionCache = sub || null;
  ...
  async function subscribe() {
    ...
    _subscriptionCache = subscription;
  ...
  // Iniciar chequeo en background apenas carga el módulo
  if (typeof document !== 'undefined') {
    document.addEventListener('DOMContentLoaded', () => refreshSubscriptionCache());
  }
