(function attachMusgoAppAuth(globalScope) {
  ...
  globalScope.handleEnablePush = async function handleEnablePush() {
    const push = globalScope.musgoPush;
    if (!push) return;
    const result = await push.subscribe();
    if (result.ok) {
      getAppAuthContext().showToast('🔔 Recordatorios de riego activados');
    } else if (result.reason === 'denied') {
      getAppAuthContext().showToast('⚠️ Permiso denegado. Habilitálos desde Configuración.');
    } else {
      getAppAuthContext().showToast('⚠️ No se pudieron activar las notificaciones');
    }
    getAppAuthContext().render();
  };

  globalScope.handleDisablePush = async function handleDisablePush() {
    const push = globalScope.musgoPush;
    if (!push) return;
    await push.unsubscribe();
    getAppAuthContext().showToast('🔕 Recordatorios desactivados');
    getAppAuthContext().render();
  };
})(window);
