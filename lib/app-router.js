(function attachMusgoRouter(globalScope) {
  function createMusgoRouter({
    ...
    const ROUTABLE_VIEWS = [...new Set([...PUBLIC_VIEWS, ...AUTHENTICATED_VIEWS])];
    const VIEW_PATHS = {
      landing: '/',
      auth: '/login',
      dashboard: '/dashboard',
    ...
    function shouldShowTopbar() {
      const state = getStateSnapshot();
      if (state.splashVisible || shouldShowPublicExperience()) return true;
      if (!isAuthenticatedMainView()) return false;
      // On the 5 main authenticated screens, mobile/tablet replaces the
      // topbar (logo + hamburger) with a sticky bottom navbar. The desktop
      // sidebar layout keeps its top strip.
      return shouldUseDesktopSidebarLayout();
    }
    ...
    function shouldShowBottomNav() {
      const state = getStateSnapshot();
      if (state.splashVisible) return false;
      if (!shouldShowAuthenticatedExperience()) return false;
      if (!isAuthenticatedMainView()) return false;
      return !shouldUseDesktopSidebarLayout();
    }
